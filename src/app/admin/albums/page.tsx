...
export default function AdminAlbumsPage() {
  ...
  const createAlbum = async () => {
    ...
    try {
      ...
      addToast("Error al crear album", "error");
    }
  };
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <FadeUp className="mb-8 md:ml-0 ml-10">
          <div className="flex items-center justify-between">
            <div>
              <h1 className={`text-2xl font-bold ${t.text}`}>Administrar Álbumes</h1>
              <p className={`text-sm ${t.textMuted} mt-1`}>{albums.length} álbum(es) en total</p>
            </div>
            <button
              onClick={() => setShowForm(!showForm)}
              className="px-4 py-2 rounded-xl text-sm font-medium text-white btn-glass-accent"
            >
        ...
        {showForm && (
          <div className={`${t.glassCard} rounded-2xl p-6 mb-6`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className={`block text-sm ${t.textMuted} mb-1`}>Año</label>
                <input
                  type="number"
                  value={newAlbum.year}
                  onChange={(e) => setNewAlbum((prev) => ({ ...prev, year: parseInt(e.target.value) }))}
                  className={`w-full px-3 py-2.5 rounded-xl border ${t.inputBorder} ${t.inputBg} ${t.text} focus:ring-2 focus:ring-blue-500 focus:outline-none`}
                />
              </div>
              <div>
                <label className={`block text-sm ${t.textMuted} mb-1`}>Título</label>
                <input
                  type="text"
              ...
              <div>
                <label className={`block text-sm ${t.textMuted} mb-1`}>Categoría</label>
                <input
                  type="text"
              ...
              <div>
                <label className={`block text-sm ${t.textMuted} mb-1`}>Descripción</label>
                <input
                  type="text"
        ...
        {albums.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button
              ...
              className="px-5 py-2 rounded-xl text-white font-medium btn-glass-accent"
            >
              Crear Álbum
            </button>
          </div>
              ...
              <StaggerItem key={album.id} className={`${t.glassCard} rounded-2xl p-5`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className={`text-sm ${t.textMuted}`}>Año {album.year}</p>
                  </div>
                  <button
                ...
                {album.description && (
                  <p className={`text-sm ${t.textMuted} mb-3`}>{album.description}</p>
                )}
                ...
                <div className="flex gap-2">
                  <Link href={`/admin/upload?album=${album.id}`} className="flex-1">
                    <button className={`w-full px-3 py-2 rounded-xl text-sm font-medium border ${t.inputBorder} ${t.text}`}>
                      Subir fotos
                    </button>
                  </Link>
                  <Link href={`/album/${album.year}`} className="flex-1">
                    <button className="w-full px-3 py-2 rounded-xl text-sm font-medium text-white btn-glass-accent">
                      Ver álbum
                    </button>
                  </Link>
                </div>
