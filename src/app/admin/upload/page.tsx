...
function UploadContent() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      addToast("Error al subir las imagenes", "error");
    } finally {
      setUploading(false);
      setUploadProgress(0);
    }
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="space-y-6">
          <div className={`${t.glassCard} rounded-2xl p-6`}>
            <label className={`block text-sm font-medium ${t.text} mb-2`}>Álbum de destino</label>
            <select
              value={selectedAlbum}
            ...
            >
              ...
              {albums.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.year} - {a.title} {a.subAlbum ? `(${a.subAlbum})` : ""}
                </option>
              ))}
            </select>
          </div>
          ...
          <div
            className={`${t.glassCard} rounded-2xl p-8`}
          ...
          >
            <div className={`border-2 border-dashed rounded-xl p-12 text-center transition-all duration-300 ${dragActive ? "border-blue-500 bg-blue-500/10 backdrop-blur-sm scale-[1.01]" : t.inputBorder}`}>
              ...
              <p className={`font-medium ${t.text} mb-1`}>Arrastra tus fotos aquí</p>
              ...
              <label
                htmlFor="file-input"
                className="inline-flex items-center px-5 py-2.5 btn-glass-accent text-white rounded-xl cursor-pointer text-sm font-medium"
              >
            ...
            <div className={`${t.glassCard} rounded-2xl p-6`}>
              ...
              <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-black/5 dark:border-white/5">
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-medium border ${t.inputBorder} ${t.text}`}
                >
                  Cancelar
                </button>
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-medium text-white ${t.accentBg} disabled:opacity-50`}
                >
          ...
          {uploading && (
            <div className={`${t.glassCard} rounded-2xl p-6`}>
              ...
              <div className={`w-full ${t.inputBg} rounded-full h-2`}>
                <div
                  className={`${t.accentBg} h-2 rounded-full transition-all duration-300`}
...
export default function AdminUploadPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-2 border-t-transparent border-gray-300" /></div>}>
      <UploadContent />
    </Suspense>
  );
}
