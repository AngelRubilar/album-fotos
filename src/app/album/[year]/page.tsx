              ...
              return (
                ...
                >
                  <div className="photo-card cursor-pointer relative" onClick={() => { setGalleryIndex(index); setGalleryOpen(true); }}>
                    ...
                    <button onClick={e => { e.stopPropagation(); deleteImage(image.id); }} className={`action-touch absolute top-2 right-2 p-1.5 rounded-lg bg-black/30 backdrop-blur-md border border-white/10 opacity-0 group-hover:opacity-100 transition-all ${t.danger}`} aria-label="Eliminar imagen">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
